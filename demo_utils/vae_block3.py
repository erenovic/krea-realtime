...
class VAEEncoderWrapper(nn.Module):
    ...
    def forward(self, z: torch.Tensor, feat_cache: List[torch.Tensor], stream: bool = False):
        ...
        for i in range(iter_):
            ...
            if i == 0 and feat_cache[0] is None:
                out = self.encoder(z[:, :, :1, :, :], feat_cache=feat_cache, feat_idx=self._enc_conv_idx)
            else:
                slice_start = i - 1
                if stream:
                    offset = 0
        ...
        for idx, layer in enumerate(self.head):
            if isinstance(layer, CausalConv3d) and feat_cache is not None:
                ...
                if fill_cache_x.shape[2] < 2 and feat_cache[idx] is not None:
                    # cache last frame of last two chunk
                    cache_x = torch.cat(
                        [feat_cache[idx][:, :, -1, :, :].unsqueeze(2).to(cache_x.device), fill_cache_x], dim=2
                    )
