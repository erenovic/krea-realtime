...
def save_bcthw_as_mp4(x, output_filename, fps=10, crf=0):
    ...
    torchvision.io.write_video(
        output_filename, x, fps=fps, video_codec="libx264", options={"crf": str(int(crf))}
    )
    return x
...
def lazy_positional_encoding(t, repeats=None):
    ...
    te = get_timestep_embedding(
        timesteps=te, embedding_dim=256, flip_sin_to_cos=True, downscale_freq_shift=0.0, scale=1.0
    )
