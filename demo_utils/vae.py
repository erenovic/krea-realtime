        ...
        for layer in self.residual:
            if isinstance(layer, CausalConv3d):
                ...
                if cache_x.shape[2] < 2 and feat_cache is not None:
                    ...
                    cache_x = torch.cat(
                        [feat_cache[:, :, -1, :, :].unsqueeze(2).to(cache_x.device), cache_x], dim=2
                    )
    ...
    def __init__(self, dim, mode):
        ...
        if mode == "upsample2d":
            self.resample = nn.Sequential(
                Upsample(scale_factor=(2.0, 2.0), mode="nearest"), nn.Conv2d(dim, dim // 2, 3, padding=1)
            )
...
class VAEDecoderWrapperSingle(nn.Module):
    ...
    def forward(self, z: torch.Tensor, is_first_frame: torch.Tensor, *feat_cache: List[torch.Tensor]):
...
class VAEDecoder3d(nn.Module):
    ...
    def forward(self, x: torch.Tensor, is_first_frame: torch.Tensor, feat_cache: List[torch.Tensor]):
        idx = 0
        out_feat_cache = []
